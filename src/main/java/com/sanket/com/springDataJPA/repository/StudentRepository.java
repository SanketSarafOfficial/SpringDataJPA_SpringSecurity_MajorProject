...
public interface StudentRepository extends JpaRepository<Student , Long> {
    ...
    // These 4 method's logic are generated by JpaRepository Interface
    ...
    @Query(
            value = "select * from tbl_student s where s.email_address = ?1",   // make sure the names of the fields (The Table name and the column name) matching with database in the Query
            nativeQuery = true
    )
    ...
    // Here we will update the first name of the student

    @Modifying      // helps in modification  to Insert , update and delete in the database
    @Transactional  // helps to achieve ACID properties (Atomicity , Consistency , Isolation , Durability)
    @Query(
...
/*
* JPQL queries uses Class Name instead of SQL DB table name for Quering purpose
* Native JPQL Query uses SQL DB table name as well as columns for Querying
*
*
* */
